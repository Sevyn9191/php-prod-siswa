...
<script>
	// $(document).ready(function(){
      ...
      var qty = $('#qty').val();
			$.ajax({
				method: 'post',
				url: "<?php echo base_url(); ?>pembayaran/byr_pros",
        data:{kwit:kd,by:biaya,amo:qty},
        dataType:'json',
				success: function(data) {
        if(data.fail){
            alert('isi field dengan benar');
          }else{
            alert('siswa dengan nama '+data.Nama+' sudah bayar dengan total '+data.Jumlah + ' Klik Print Jika Tidak ada tambahan data kembali.');
          }
          // bootbox.confirm("Do you really want to delete record?");
          // alert('jans'+kd);
				}
			});
		});
...
<script>
		$("#oke-pret").on('click',function(event){
      var kd = $('#kwitansi').val();
      event.preventDefault();
      var g = confirm('are you sure ?');
	    if(g == true){
        window.location.href="<?php echo base_url();?>pembayaran/print_yuk/"+kd;
      }else{
        alert('Oke');
      }
		});
